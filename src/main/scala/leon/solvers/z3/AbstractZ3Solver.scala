...
trait AbstractZ3Solver extends Solver {
  ...
  protected[leon] def toZ3Formula(expr: LeonExpr, initialMap: Map[Identifier, Expr] = Map.empty): Expr = {
    ...
    def rec(ex: LeonExpr): Expr = ex match {
      ...
      case u : UnitLiteral =>
        typeToSort(UnitType) // Make sure it is defined
        constructors.toB(UnitType)()
      ...
      case ElementOfSet(e, s) =>
        z3.mkSetMembership(rec(e), rec(s))

      case SubsetOf(s1, s2) =>
        z3.mkSetSubset(rec(s1), rec(s2))

      case SetIntersection(s1, s2) =>
        z3.mkSetIntersection(rec(s1), rec(s2))
      ...
      t match {
        case i: IntNum =>
          tpe match {
            ...
            case CharType  => CharLiteral(i.getInt64.toChar)
        ...
        case bv: BitVecNum =>
          ...
          // but .toLong will give a large enough long and .toInt will just keep the lower bits
          tpe match {
            case IntegerType => InfiniteIntegerLiteral(bv.getBigInteger)
            case Int32Type => IntLiteral(bv.getLong.toInt)
            case CharType => CharLiteral(bv.getLong.toChar)
          }
        case r: RatNum =>
          ...
          if(functions containsB decl) {
            ...
            tpe match {
              ...
              case MapType(from, to) =>
                ...
                if (default.getType != library.noneType(to)) {
                  unsupported(r, "Solver returned a co-finite Map which is not supported.")
                }

                val leonSome = library.Some.get.typed(Seq(to))

                val elems = rawElems.collect {
                  case (k, CaseClass(`leonSome`, Seq(x))) => k -> x
                }

                FiniteMap(elems, from, to)

              case tpe @ SetType(dt) =>
                val r @ RawArrayValue(`dt`, rawElems, default) = rec(t, RawArrayType(dt, BooleanType))
                  // We expect a RawArrayValue with keys in from and values in Option[to],
                  // with default value == None
                  if (default != BooleanLiteral(false)) {
                    unsupported(r, "Solver returned a co-finite Set which is not supported.")
                  }

                  val elems = rawElems.collect{ case (e, BooleanLiteral(true)) => e }.toSet

                  FiniteSet(elems, dt)

              case _ =>
                unsound(t, "Z3 returned unexpected AST")
            }
          }
        case _ =>
          unsound(t, "Z3 returned unexpected AST")
